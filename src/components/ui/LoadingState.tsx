      ...
      <div className="space-y-3">
        {Array.from({ length: rows }).map((_, i) => (
          <div key={i} className="flex items-center gap-3">
            <div className="h-9 w-9 shrink-0 animate-pulse rounded-full bg-[#eceae4]" />
            <div className="flex-1 space-y-2">
              <div className={cn("h-3 animate-pulse rounded bg-[#eceae4]", i % 2 ? "w-1/3" : "w-1/2")} />
              <div className="h-3 w-2/3 animate-pulse rounded bg-[#f1efe9]" />
            </div>
          </div>
        ))}
      </div>
